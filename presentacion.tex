...
{\small Asesor: José Guadalupe Martínez Bernal}
\end{beamercolorbox}
...
\begin{equation*}
    ...
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        \path[every node]
        (1) edge[->,>=latex,bend right, line width=1pt] node[below] {$1$} (2);
        \path[every node]
        (2) edge[->,>=latex,bend right, line width=1pt] node[above] {$\frac{1}{2}$} (1);
        \path[every node]
        (2) edge[->,>=latex,bend right, line width=1pt] node[below] {$\frac{1}{2}$} (3);
        \path[every node]
        (3) edge[->,>=latex,bend right, line width=1pt] node[above] {$\frac{1}{2}$} (2);
        \path[every node]
        (3) edge[->,>=latex,bend right, line width=1pt] node[below] {$\frac{1}{2}$} (4);
        \path[every node]
        (4) edge[->,>=latex,bend right, line width=1pt] node[above] {$\frac{1}{2}$} (3);
        \path[every node]
        (4) edge[->,>=latex,bend right, line width=1pt] node[below] {$\frac{1}{2}$} (5);
        \path[every node]
        (5) edge[->,>=latex,bend right, line width=1pt] node[above] {$1$} (4);
    \end{tikzpicture}
\end{equation*}
...
\begin{equation*}
    ...
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        \path[every node]
        (1) edge[<->,>=latex,line width=1pt] node[above right] {$\frac{1}{4}$} (2);
        \path[every node]
        (1) edge[<->,>=latex,line width=1pt] node[below right] {$\frac{1}{4}$} (5);
        \path[every node]
...
\begin{frame}{Distribuciones reversibles}
    ...
    \begin{thm}
        Si $\pi$ es reversible para $P$, entonces $\pi$ es una distribución
        estacionara para $P$.
    \end{thm}
    
    \begin{align*}
        (P\pi)_i&= \sum_{j=1}^N P_{ji}\pi_i\\
...
\begin{frame}{Cadena de Metropolis-Hastings}
    ...
    Para producir la \emphdef{cadena de M-H} $M$. De manera que $\pi$ es reversible
    para $M$.
...
\]
M-H es un {\it ajuste} a $Q$ para obtener la reversibilidad.
}{es decir,
\[
...
%\end{thm}
    El siguiente algoritmo describe una caminata aleatoria asociada a la
    cadena de Metropolis-Hastings.

\bigskip

    \begin{enumerate}
        \item
        Supongamos que la caminata está ubicada en el estado $j$ en este momento.
        ...
        \item
        Si $\pi_j Q_{ij}>\pi_i Q_{ji}$, lanzamos una moneda sesgada con probabilidad $\pi_i
...
{\it La caminata favorece incrementos en $\pi$ con probabilidad de $j\rightarrow i$
alta y $i\rightarrow j$ baja.}
\end{frame}

\begin{frame}{Puntos importantes de M-H}
\begin{itemize}
    \item
...
\bigskip
    ...
    \begin{enumerate}
        ...
        \item
        En caso contrario, lanzamos una moneda sesgada con probabilidad
...
\begin{frame}{Ejemplo del alg. de M-H}
\begin{equation*}
    \begin{tikzpicture}[scale=.75, baseline={([yshift=-.5ex]current bounding box.center)}]
        \node[my state] (1) at (0,2) {$1$};
        ...
        \path[every node]
        (1) edge[->,>=latex,line width=1pt,bend left=20] node[above] {\small $60/100$} (2);
        \path[every node]
        (2) edge[->,>=latex,line width=1pt,bend left=20] node[below] {\small $99/100$} (1);
        \path[every node]
...
\alt<2>
{
$1\xrightarrow{60/100} 2$, como $Q_{12}\pi_1> Q_{21}\pi_2$, 
\begin{align*}
    \text{lanzamos moneda }\underbrace{10/11}_{2},\quad \underbrace{1/11}_{1}
\end{align*}
}
{
$2\xrightarrow{99/100} 1$ es aceptado porque $Q_{21}\pi_2\leq Q_{12}\pi_1$
}
\end{frame}
...
\begin{frame}{Caminatas en la malla $\bZ^d$}

\emphdef{Caminata simple}: Digráfica con pesos positivos
donde cada arista que sale de $i$ tiene peso igual
a $1/d_i$, $d_i$ es el grado del vértice.
...
\begin{tikzpicture}[scale=1.50]
    ...
    node[anchor=east] {\large $\frac{1}{4}$};
    \draw (0,0) -- (1,0) ;
    \filldraw[black] (0.5,0) circle(0pt)
    node[anchor=north] {\large $\frac{1}{4}$};
    \draw[dotted] (1,0) -- (2,0) ;
    \draw[dotted] (2,0) -- (3,0) ;
    \draw (0,1) -- (1,1) ;
    \draw[dotted] (1,1) -- (2,1) ;
...
\begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
    \node[my state] (1) at (0,0) {$s$};
    \node[my state] (3) at (1.5,0) {$3$};
    \node[my state] (2) at (1,1.5) {$2$};
    \node[my state] (4) at (2.5,1.5) {$t$};

    \path[every node]
    (1) edge[-] node[left] {\small $\omega_{s2}$} (2);
    \path[every node]
    (1) edge[-] node[below] {\small $\omega_{s3}$} (3);
    \path[every node]
    (1) edge[-] node[below] {} (4);
    \path[every node]
    (2) edge[-] node[above] {} (4);
...
\[
    \sum_{j=1}^k \phi(v_1)-\phi(v_{j+1}) = 0
\]
\bigskip
...
La estructura de red eléctrica es una solución al problema de Dirichlet discreto
en la gráfica $(\Gamma, \omega)$ cuando la frontera es $\{s,t\}$.
...
Para una función continua en un abierto $A\subset\bC$...
\[
    h(a)=\frac{1}{2\pi}\int_0^{2\pi}h(a+re^{i\theta})\,d\theta.
\]
Nos fijamos en la cadena de Markov con matriz de transición
\[
    P_{uv}=\frac{w_{uv}}{W_v},\qquad\text{donde}\qquad
    W_v=\sum_{u\in V}w_{vu}.
\]

\bigskip
\bigskip

$f\colon V\rightarrow \bR$ es \emphdef{armónica} en $U\subset V$ si
\[
...
\begin{frame}{Ejemplo básico de circuito eléctrico}
\begin{equation*}
\begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
    ...
    \path[every node]
    (s) edge[-] node[above] {$1/2$} (1);
    \path[every node]
    (1) edge[-] node[above] {$1/2$} (t);
\end{tikzpicture},
\qquad\qquad
P=
\begin{bmatrix}
...
\begin{equation*}
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        (s) edge[-] node[above] {} (1);
        \path[every node]
        (1) edge[-,bend left] node[above] {$r_1$} (2);
        \path[every node]
        (1) edge[-,bend right] node[below] {$r_2$} (2);
        \path[every node]
        (2) edge[-] node[below] {} (t);
    \end{tikzpicture},
    \qquad 
    \begin{tikzpicture}[baseline={([yshift=0ex]current bounding box.center)}]
        ...
        (s) edge[-] node[above] {} (1);
        \path[every node]
        (1) edge[-] node[below] {$\frac{r_1r_2}{r_1+r_2}$} (2);
        \path[every node]
        (2) edge[-] node[below] {} (t);
    \end{tikzpicture}
\end{equation*}
...
\begin{equation*}
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        (s) edge[-] node[above] {} (1);
        \path[every node]
        (1) edge[-] node[above] {$r_1$} (2);
        \path[every node]
        (2) edge[-] node[above] {$r_2$} (3);
        \path[every node]
        (3) edge[-] node[below] {} (t);
    \end{tikzpicture},
    \qquad 
...
La corriente eléctrica $i$ es un ejemplo de $(s,t)$-flujo.
...
Notemos que
\begin{equation*}
    J_s+J_t = \sum_{u\in V}J_u = \sum_{u,v\in V}j_{uv}
    = \frac{1}{2}\sum_{u,v\in V}(j_{uv}+j_{vu}) = 0
\end{equation*}
por lo que $J_s = -J_t$. Convenimos en que $J_s>0$.
...
Con la identidad 
\begin{prop}[Principio de conservación de la energía]
    Para $\psi\colon V\rightarrow \bR$ y $j$ un $(s,t)$-flujo se cumple que
    \[
        (\psi(t)-\psi(s))J_s = \frac{1}{2}\sum_{u,v\in V}(\psi(v)-\psi(u))j_{uv}
    \]
    
\end{prop}
en una red eléctrica con voltaje $\phi$ y corriente $i$ se tiene que
\[
...
\[
    \Reff =\frac{E(i)}{\cI_s^2} =\cdots = \frac{\phi(t)-\phi(s)}{\cI_s}
\]
...
\begin{align*}
    ...
    &\\
    V_{n+1} &= \Delta_n\ \cup\ \bar{V\setminus\Delta_n},
    \quad \bar{V\setminus\Delta_n}=I_n\\
\end{align*}
...
\begin{center}
    \begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
        ...
        (1) edge[-] node[above] {} (2);
        \path[every node]
        (2) edge[-] node[above] {$\frac{1}{2}$} (3);
        \path[every node]
        (3) edge[-] node[above] {$\frac{1}{2}$} (4);
        \path[every node]
        (4) edge[-] node[above] {} (5);
        \path[every node]
        (5) edge[-] node[above] {} (e);
...
\begin{frame}{Demostración del T. de Polya}{\large Caso $d=2$}
...
\begin{center}
    \begin{tikzpicture}
        ...
        \path[every node]
        (0) edge[-,bend left=40] node[above] {} (1);
        \path[every node]
        ...
        \path[every node]
        (1) edge[-,bend right=40] node[below] {$8$} (2);
        \path[every node]
        (2) edge[-,bend left=40] node[below] {$\vdots$} (3);
        \path[every node]
        (2) edge[-,bend right=40] node[below] {$12$} (3);
    \end{tikzpicture}
\end{center}
\bigskip
\bigskip
Otro dibujo aquí (cómo se obtiene la gráfica de arriba)
\end{frame}

\begin{frame}{Demostración del T. de Polya}{\large Caso $d=2$}
Para cada $n$, tenemos que
\[
    \sum_{n=1}^\infty \frac{4}{8n-4} \leq \Reff(n)
\]
\bigskip
Por lo que
\[
...
\begin{frame}{Demostración del T. de Polya}{\large Caso $d=3$}
...
\begin{tikzpicture}
    ...
    \draw[gray!30,->] (0,0) -- (8,0);
    \draw[gray!30,->] (0,0) -- (0,2);
    \draw[gray!30,->] (0,0) -- (-1.5,-1.5);
    ...
    \draw[dotted] (4.75,-1.25) -- (5.25,-0.75);
    \draw[dotted] (5.25,-0.75) -- (5.25,1.25);
    \draw[dotted] (5.25,-0.75) -- (7.25,-0.75);

    \draw[dashed,blue!40] (0,0) -- (4.75,0.75);
    \draw[dashed,blue!40] (0,0) -- (4.75,-1.25);
    \draw[dashed,blue!40] (0,0) -- (5.25,1.25);
    \draw[dashed,blue!40] (0,0) -- (5.25,-0.75);
...
\end{itemize}
\bigskip
Sólo falta ver que tiene energía finita. Unas estimaciones...
...
\begin{align*}
    ...
    \mathcolor{red}{P(z) }& \mathcolor{red}{=\sum_{n=0}^\infty p_nz^n}
\end{align*}
}{
\[
    p = \sum_{n=0}^\infty p_n = \sum_{n=1}^\infty p_n, \qquad p_0=0
...
\mathcolor{red}{
\[
    Q(z) = \sum_{n=0}^\infty q_nz^n = 1 + \sum_{n=1}^\infty q_nz^n,\qquad q_0=1
\]
}
