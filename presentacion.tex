...
en un vértice y avanza en cada paso a un vértice vecino aleatorio.
...
\bigskip
    ...
    \begin{enumerate}
        ...
        \item
        En caso contrario, lanzamos una moneda de Bernoulli con probabilidad
...
\begin{tikzpicture}[baseline={([yshift=-.5ex]current bounding box.center)}]
    ...
    \path[every node]
    (s) edge[-] node[left] {$\omega_{s2}$} (2);
    \path[every node]
    (s) edge[-] node[below] {$\omega_{s3}$} (3);
    \path[every node]
    (s) edge[-] node[below] {} (t);
    \path[every node]
    (2) edge[-] node[above] {} (t);
...
\[
    \sum_{k=1}^n \frac{4}{8k-4} \leq \Reff(n)
\]
\bigskip
Por lo que
\[
...
\begin{frame}{Demostración del T. de Polya}{Caso $d=3$}
